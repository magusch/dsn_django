...
def refresh_posting_time(self, request, queryset):
    for query in queryset:
        last_post = Events2Post.objects.exclude(status='Posted').filter(queue__lt=query.queue)\
...
def last_post_date():
    last_post_event = Events2Post.objects.exclude(status='Posted').order_by('-post_date').first()
    if last_post_event:
...
def move_event_to_post(Events_model): #ToDO: remove from here
    event2post_list = ['event_id', 'title', 'post', 'image', 'url',
                       'price', 'address', 'explored_date', 'from_date', 'to_date']
...
def good_post_time(last_post_time):
    ...
    if post_time_query_first:
        ...
        post_time = next_day.replace(hour=post_time.posting_time_hours-current_tz_int,
                                     minute=post_time.posting_time_minutes, second=0, microsecond=0)
    return post_time
