        ...
        completion = self.client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user",
                 ...
                 Текст мероприятия не должен содержать какие-то точные даты, по возможности перевести их в указания дней недель. Также Убрать все ссылки, спец-символы и другие мешающие вещи из текста. Из всего текста выделить основную мысль и выложить её в одном абзаце (1-3 предложения). Стиль написания должен быть упрощённым и понятным, оставить капельку любопытсва если оно присутсововало в оригинальном тексте. Текст не должен быть от первого лица. Все местоимения перефразировать в третье лицо ("они что-то сделали"). 
                 Обязательно выделить категорию мероприятия, можно взять из заголовка. Выделить несколько важных тегов мероприятия. Результат выдать в виде названия информации (заголовк, текст) затем элемент в виде '=>', результат и в конце поставить точку с запятой (;).
                 МЕРОПРИЯТИЕ:
